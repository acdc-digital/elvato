...
function loadEnv() {
  const envPath = path.join(process.cwd(), "admin", ".env");
...
async function main() {
  ...
  // Map top-level product category IDs to target collection handles.
  ...
  const candidates = await client.query(
    ...
    `,
    [
      ["Featured", ...targetHandles],
      Object.keys(categoryToCollectionHandle),
    ]
  );
