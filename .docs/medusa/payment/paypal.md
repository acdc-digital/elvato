...
pnpm
npm
yarn add @paypal/paypal-server-sdk
...
Determine the payment intent based on the autoCapture option. By default, payments are authorized for later capture.
Create a PayPal order request with the payment details.
...
Extract the PayPal order ID from the data field. This is the same data.order_id you returned in the initiatePayment method.
If the autoCapture option is enabled, you capture the PayPal order.
Otherwise, you authorize the PayPal order.
...
Extract the PayPal authorization ID from the data field. This is the same data.authorization_id you returned in the authorizePayment method.
...
Extract the PayPal capture ID from the data field. This is the same data.capture_id you returned in the capturePayment method.
Extract the amount and currency code from the input.
...
Extract the PayPal order ID from the data field. This is the same data.order_id you returned in the initiatePayment method.
Update the PayPal order amount.
...
Extract the PayPal order ID from the data field. This is the same data.order_id you returned in the initiatePayment method.
Retrieve the PayPal order details.
Map the PayPal order status to Medusa's PaymentSessionStatus.
...
rawBody: The raw body from the webhook request as a string or buffer.
In the method, you:
...
Verify the webhook signature using the verifyWebhookSignature method.
Extract the event_type from the webhook payload to determine the type of event.
Extract the session ID and amount from the webhook resource.
...
The name of the module that this provider belongs to, which is Modules.PAYMENT in this case.
An object with a required property services indicating the Module Provider's services.
...
In the Webhook URL field, enter your ngrok URL followed by /hooks/payment/paypal_paypal. For example: https://abcd1234.ngrok.io/hooks/payment/paypal_paypal.
...
npm
yarn dev
Open the Medusa Admin dashboard in your browser at http://localhost:9000/app and log in.
...
Retrieve the PayPal Client ID from the environment variable NEXT_PUBLIC_PAYPAL_CLIENT_ID.
Set the initial options for the PayPal SDK, including the client ID, currency, and intent (capture or authorize).
...
Wrap the children components with the PayPalScriptProvider component from the PayPal React SDK, passing the initial options.
...
Click on the email ending with @personal.example.com to view the account details.
