...
*   OAuth—eBay APIs use the OAuth 2.0 protocol for application and user authorization. OAuth is the industry standard for assuring your online transactions are secure. You must provide a valid access token for each request you make to the eBay interfaces. OAuth access tokens verify to eBay that a request is coming from a valid application and that the application has the user's authorization to carry out the requests. You can also use OAuth tokens with eBay Traditional APIs.
    ...
    **Important:** If you use Traditional APIs like the Trading API, we recommend that you use OAuth, but eBay's older Auth'n'Auth process is still available for older applications.
    ...
    You can see the OAuth scopes for your application in [Application Keys](/my/keys "Link to Application Keys page").
...
[The assignment of scopes](#assignment)  
[Scopes and access](#access)  
[Specifying scopes when minting access tokens](#specifying-scopes)  
[Getting the list of scopes assigned to your application](#Getting)  
[Scopes and grant flows](#scopes-and-flows)  
[Scopes and refresh tokens](#refresh-tokens)
...
![Banner image](/cms/img/authorization/oauth-link.png "Link to OAuth scopes")
...
![Banner image](/cms/img/authorization/oauth-scopes-available.png)

The request you use to generate the new token must include a list of scopes that allows access to _all_ the methods you plan to call with the token.
...
When using the [Identity API](/api-docs/sell/identity/resources/methods) to mint tokens, the scopes belonging to an application keyset are passed in through a scope query parameters. See  [The client credentials grant flow](/api-docs/static/oauth-client-credentials-grant.html) and [The authorization code grant flow](/api-docs/static/oauth-authorization-code-grant.html) for detailed information on how to pass in the scopes through the scopes query parameter.
...
2.  Click the **User Tokens** link that's displayed next to the Client ID of the environment you want to target:
    ...
    ![Banner image](/cms/img/authorization/oauth-get-user-tokens.png "View sample request")
...
3.  Under the **Get a Token from eBay via Your Application** heading, after configuring an RuName, an **RuName** box similar to the following displays:
    ...
    ![Banner image](/cms/img/authorization/runame.png)
...
When you copy the scopes in this manner, you can end up using a larger set of scopes than is needed by your application. The user scopes imply a permission grant, and users must _consent_ to all the permissions your scopes represent before they can use your application.
...
[The client credentials grant flow](/api-docs/static/oauth-client-credentials-grant.html) is used to mint application tokens, which can be used if the application is accessing or working with resources and data that is not specific to an eBay user. Good examples of methods that require application tokens are metadata or taxonomy calls.

 [The authorization code grant flow](/api-docs/static/oauth-authorization-code-grant.html) is used to mint user tokens which are used for methods that do post or return data that is specific to an eBay user. There are a lot more methods that require user tokens than application tokens. The **OAuth scope** section in each method's reference page indicates which tokens are needed for that method.

##### Scopes and refresh tokens

When creating a User access token, you must supply a list of scopes in your consent request. (See [Getting the list of scopes assigned to your application](/api-docs/static/oauth-scopes.html#Getting)). For more details, see [The authorization code grant flow](/api-docs/static/oauth-authorization-code-grant.html).
...
[Minting access tokens](#minting)

eBay APIs use the [OAuth 2.0 protocol](https://tools.ietf.org/html/rfc6749) for application and user authorization. OAuth is the industry standard for assuring your online transactions are secure and you must provide a valid access token for each request you make to the eBay REST interfaces.

OAuth _access tokens_ verify to eBay that a request is coming from a valid application and that the application has the user's authorization to carry out the requests.
...
The eBay token service generates, or _mints_, access tokens via two different grant flows:

*   **Client credentials grant** flow mints a new _Application access token_ that you can use to access the resources owned by the application.
*   **Authorization code grant** flow mints a new _User access token_ that you can use to access the resources owned by the user.
...
*   [The client credentials grant flow](/api-docs/static/oauth-client-credentials-grant.html)
*   [The authorization code grant flow](/api-docs/static/oauth-authorization-code-grant.html)

###### Getting your redirect\_uri value

The requests to get user access tokens require a **redirect\_url** value that is used for authorization, as well as a URL to redirect the user to after they have completed the permissions grant request. Instead of a URL, the OAuth flow requires a custom _RuName_ value that eBay generates and assigns to your application.
...
2.  Click the **User Tokens** link that's displayed next to your Client ID:
    ...
    ![Banner image](/cms/img/authorization/oauth-get-user-tokens.png)
...
3.  Click the **Get a Token from eBay via Your Application** drop down, the following screen displays:
    ...
    ![Banner image](/cms/img/authorization/oauth-get-a-token-dialog1.png)
    
4.  If you have not yet created an RuName (also known as an _eBay Redirect URL name_), click the link labeled **You have no Redirect URLs. Click here to add one.**
...
5.  Complete the form to confirm your contact information, then click the **Continue to create RuName** button.
    ...
    ![Banner image](/cms/img/authorization/oauth-runame-a.png)  
...
![Banner image](/cms/img/authorization/oauth-runame-u.png)  
The RuName value for the Sandbox environment
...
Before encoding the credentials, combine them by listing the **client\_id** value, a colon, then the **secret** value.
...
Basic <_B64-encoded\_oauth\_credentials_\>
...
![Banner image](/cms/img/authorization/oauth-ui-app-token.png)  
...
Access tokens are _short lived_—they expire quickly for security reasons—while refresh tokens are valid for an extended period of time. Refresh tokens are limited in functionality, however, and you can only use them to get a new access token (you cannot use refresh tokens to authorize requests).
...
To help prevent nefarious use of the eBay APIs, each application has a limit on the number of requests it can make to the OAuth endpoint in any one 24-hour day. The number of requests allocated to an application is called the _rate limit_ for that application.

The OAuth rate limits are assigned to the endpoint that an application uses to mint OAuth access tokens (`https://api.ebay.com/identity/v1/oauth2/token`), and the rate limits are different for each **grant\_type** value. The following table shows the rate limits for the different types of **grant\_type** values:
...
[Sequence for getting and using an application access token](#sequence)  
[Configuring the request](#configure)  
[The client credentials grant request](#request)

This topic describes how to mint OAuth access tokens using the **client credentials grant flow**.

The access token retrieved from this process is called an _Application access token_.
...
![Banner image](/cms/img/authorization/client-credentials.png)  
 _Sequence diagram for generating an Application access token_
...
*   **Authorization** – The word "`Basic` " followed by your Base64-encoded OAuth credentials (`<client_id>**:**<client_secret>`).
    ...
    For details, see [Generating your Base64-encoded credentials](/api-docs/static/oauth-base64-credentials.html).
...
*   Set **grant\_type** to `client_credentials`.
...
Configure your request using the following call specifics:
  ...
  **HTTP headers:**
    ...
    Authorization = Basic <_B64-encoded-oauth-credentials_\>
...
curl -X POST 'https://api.sandbox.ebay.com/identity/v1/oauth2/token' \\
  ...
  -d '**grant\_type**\=client\_credentials&**scope**\=https%3A%2F%2Fapi.ebay.com%2Foauth%2Fapi\_scope %20https%3A%2F%2Fapi.ebay.com%2Foauth%2Fapi\_scope%2Fbuy.item.bulk'
...
When you issue a client credentials grant request, eBay returns a JSON object that contains an Application access token, as shown in this response:
  ...
  {
    **"access\_token": "v^1.1#i^1#p^1#r^0#I^3#f^0#t^H4s ... wu67e3xAhskz4DAAA",**
    "expires\_in": 7200,
    "token\_type": "Application Access Token"
  }

To use an access token to authorize an API request, pass the token value in the Authorization HTTP header.

In the above example, the **expires\_in** element is set to 7,200 seconds, meaning this token is valid for two hours from the time it was generated. For continued access after the token expires, you must mint a new token.

**Important!** Access tokens _must_ be treated as confidential and must not be shared or exposed publicly. For best performance and security, applications should store this token in a static variable and re-use the token while it is valid.

## The authorization code grant flow

[Getting user consent](#userconsent)  
[Exchanging the authorization code for a User access token](#exchangingtheauthorizationcode)  
[Using a refresh token to update a User access token](#usingarefreshtokentoupdate)
...
![Banner image](/cms/img/authorization/authorization-code.png)  
 _Sequence diagram for generating a User access token_

Getting a new access token for a user through the authorization code grant flow is a two-step process. First get consent from the eBay user to make API calls on their behalf (see  [Getting user consent](/api-docs/static/oauth-consent-request.html)), and then generate the User access token for each user (see [Exchanging the authorization code for a User access token](/api-docs/static/oauth-auth-code-grant-request.html)).
...
The **client\_id** value for the environment you're targeting.

**Occurrence:** Required

`locale`

The **locale** parameter to localize the OAuth consent page for the marketplace you're targeting. For example, set `locale=de-DE` for Germany. See the **Locale Support** column of the [Marketplace ID values](/api-docs/static/rest-request-components.html#marketpl) table for the supported locales.
...
For details, see [Getting your redirect\_uri value](/keystone/guides/oauth-redirect-uri.html).

**Occurrence:** Required

`response_type`

Set to "`code`" to have eBay generate and return an _authorization code_.
...
For details, see [Using OAuth to access eBay APIs](/api-docs/static/oauth-scopes.html#specifying-scopes)
...
GET https://auth.sandbox.ebay.com/oauth2/authorize?
	client\_id=_<app-client-id-value>_& 
	locale=_<locale-value>_&          // optional
	prompt=login                    // optional
	redirect\_uri=_<app-RuName-value>_&
	**response\_type=code**&
	scope=_<scopeList>_&              // a URL-encoded string of space-separated scopes
	state=_<custom-state-value>_&     // optional
...
The **Grant Application Access** page displays information about the items you're asking the user to grant access permission for your application. The page contains an **Agree and Continue** button that the user can click to indicate _consent_ (it also contains a **Not now** button to indicate denial).

The **Grant Application Access** page has several links that provide information about the grant request and links that forward the user back to the seller's application. The seller configures much of the links and content on this page through their eBay Developer Program account. (See [Getting your redirect\_uri value](/keystone/guides/oauth-redirect-uri.html) for more details.)

![Banner image](/cms/img/authorization/grant-application-access-page.png)  
An example Grant Application Access page

The consent request

If the user grants consent by clicking the "**I Agree**" button on the **Grant Application Access** page, eBay redirects them back to the seller's **Accept URL** page (configured with the seller's RuName). The redirect back to your application includes an _authorization code_ , which indicates the user's consent.
  ...
  https://www.example.com/acceptURL.html?
    state=<_client\_supplied\_state\_value_\>&
    **code=v%5E1.1% ... NjA%3D**&
    expires\_in=299
...
For details, see [Exchanging the authorization code for a User access token](/keystone/guides/oauth-auth-code-grant-request.html).

##### Exchanging the authorization code for a User access token

When a user grants your application the authorization to take action on their behalf, eBay returns an _authorization code_ that contains the user's consent for the specified scopes. Use the authorization code in a `POST` request that's commonly known as an _authorization code grant request_. This request gets a User access token and its associated refresh token.
...
*   **Authorization** – The word `Basic` followed by your Base64-encoded OAuth credentials (`<client_id>`:`<client_secret>`)
    ...
    For details, see [Generating your Base64-encoded credentials](/keystone/guides/oauth-base64-credentials.htm).
...
*   Set **grant\_type** to `authorization_code`.
*   Set **redirect\_uri** to the _RuName_ value assigned to your application and the environment you're targeting.
    
    For details, see [Getting your redirect\_uri value](/keystone/guides/oauth-redirect-uri.htm).
    
*   Set **code** to the URL-encoded _authorization code_ value returned to you by eBay when the user granted their consent.
    
    For details, see [Getting the third-party's consent](/keystone/guides/oauth-consent-request.htm).
...
Configure your request using the following call specifics:
  ...
  **Request body:**
    ...
    code=<_authorization-code-value_\>
    redirect\_uri=<_RuName-value_\>
...
curl -X POST 'https://api.sandbox.ebay.com/identity/v1/oauth2/token' \\
  ...
  -d '**grant\_type**\=authorization\_code&
      ...
      **redirect\_uri**\=Davy\_Developer-DavyDeve-DavysT-euiukxwt'
...
When you issue the authorization code grant request, eBay returns a JSON object containing the fields shown here:
  ...
  {
    **"access\_token": "v^1.1#i^1#p^3#r^1...XzMjRV4xMjg0"**,
    "expires\_in": 7200,
    "refresh\_token": "v^1.1#i^1#p^3#r^1...zYjRV4xMjg0",
    "refresh\_token\_expires\_in": 47304000,
    "token\_type": "User Access Token"
  }
...
The **expires\_in** value returned with the newly-minted User access token indicates the number of seconds that token is valid.
...
While User access tokens are _short-lived_, the associated **refresh\_token** is a _long-lived_ value that you can use to update an expired User access token. This means you do not have to get the user's consent each time you need a new User access token.

To update the access token after it expires, see [Using a refresh token to update a User access token](/keystone/guides/oauth-refresh-token-request.html).

##### Using a refresh token to update a User access token

When you mint a new User access token, the access token is returned along with a _refresh token_, which you can use to renew the User access token for the associated user. A **refresh token request** mints an access token that contains the same authorization properties as the original access token.
...
![Banner image](/cms/img/authorization/refresh-token.png)  
 _Sequence diagram for generating a User access token with a refresh token_
...
*   **Authorization** – The word `Basic` followed by your Base64-encoded OAuth credentials (`<client_id>`:`<client_secret>`)
    ...
    For details, see [Generating your Base64-encoded credentials](/keystone/guides/oauth-base64-credentials.html).
...
*   Set **grant\_type** to `refresh_token`.
*   Set **refresh\_token** to the refresh token value returned from the authorization code grant request.
    
    To get a refresh token, you must mint a new User access token.  
    This process starts with [Getting the user's consent](/keystone/guides/oauth-consent-request.html).
    
*   Set **scope** to the same URL-encoded list of scopes that you used in the original consent request.
    
    For details on scopes, see  [Using OAuth to access eBay APIs](/api-docs/static/oauth-scopes.html#specifying-scopes).
...
To update an expired User access token using a refresh token request, configure the target URL, HTTP headers, and request payload as shown here:
  ...
  **Request body:**
    ...
    refresh\_token=<your-_refresh-token-value_\>
    scope=<_scopeList_\>   // a URL-encoded string of space-separated scopes
...
curl -X POST 'https://api.sandbox.ebay.com/identity/v1/oauth2/token' \\
  ...
  -d '**grant\_type**\=refresh\_token&
      **refresh\_token**\=v^1.1#i^1#p^3# ... fMSNFXjEyODQ=&
      **scope**\=https://api.ebay.com/oauth/api\_scope/sell.account%20
            https://api.ebay.com/oauth/api\_scope/sell.inventory'
...
When you issue the refresh token request, eBay mints a fresh access token and sends it back in a JSON response similar to the following:
  ...
  {
    **"access\_token": "v^1.1#i ... AjRV4yNjA=",**
    "expires\_in": 7200,
    "token\_type":"User Access Token"
  }
...
You can continue to use the refresh token to mint new User access tokens for a specific user, as long as the refresh token associated with their account is valid. Note, however, that if the long-lived refresh token expires, or if you update the scopes needed by your app, you must update both the access token and the refresh token, starting with [Getting the third-party consent](/keystone/guides/oauth-consent-request.html).
...
[Life Span and Uniqueness](#span-and-uniqueness)  
[Hard Expiration Warning Element](#HardExpirationWarning)  
[Revoked tokens](#Token)

There are two application use cases, or models. The models differ only by the number of users supported by the application.

In the **Single User Model**, the application supports only a single user. In this model, you need only one Auth'n'Auth token. You can generate a single token for your application's user with the [User Tokens (eBay Sign-In)](https://developer.ebay.com/DevZone/account/tokens/) page in your developer account. An Auth'n'Auth token created in the Production environment is associated with an eBay user ID, and an auth'n'auth token created in the Sandbox environment is associated with a Sandbox test user.
...
Tokens are valid for 18 months across multiple sessions of the application. Seven (7) days before a token is due to expire, eBay returns the expiration date in the [HardExpirationWarning Element](/api-docs/static/understand_auth_n_auth_tokens.html#HardExpirationWarning) field in the response of all calls the application makes on behalf of that eBay user. When your application detects this field, it must redirect the user to the sign-in page on the eBay site by the date in this warning, or the token ceases to work as a means to authenticate that eBay user.
...
[Client/Desktop Applications](#Option)  
[Web/Server Applications](#Option2)  
[Getting a Token via Fetch Token](#Getting)  
[Using Tokens](#Using)
...
2.  Your application makes a **GetSessionID** request to eBay, and receives a session ID (**SessionID**).
    ...
    **GetSessionID** is described in reference detail at [GetSessionID](/Devzone/XML/docs/Reference/eBay/GetSessionID.html).
...
3.  Use the following URL to redirect a user to the eBay sign-in page:
    ...
    The URL must include `&SessID_=SessionID_ and &RUName=` _RUName_. _SessionID_ identifies the user and _RUName_ the application. The SessionID must be URL-encoded in the sign-in URL.
    
    The equivalent URL for the Sandbox is:
    
    `[https://signin.sandbox.ebay.com/ws/eBayISAPI.dll](https://signin.sandbox.ebay.com/ws/eBayISAPI.dll)`
    
4.  `SignIn&RUName=` _RUName_ `&SessID=` _SessionID_. The user signs in on the eBay sign-in page.
...
![Banner image](/cms/img/authorization/gettingtokensgetting_tokens_for_applications_with_multiple_users4.jpg)
...
2.  Your application makes a **GetSessionID** request to eBay.
    ...
    **GetSessionID** is described in reference detail at [GetSessionID.](https://developer.ebay.com/Devzone/XML/docs/Reference/eBay/GetSessionID.html)
...
4.  Your application constructs an eBay sign-in URL that sends the user to the eBay sign-in page and consent form.
    ...
    In the URL, _SessionID_ identifies the user and _RUName_ the application. _SessionID_ must be URL-encoded in the sign-in URL. It is the session ID that was obtained by calling **GetSessionID**.
...
See [Getting a Token via FetchToken](/api-docs/static/get_auth_n_auth_tokens.html#Getting).

With one-time setup complete, your application can respond to prospective users and get tokens for them.

If the application only has one user, the same process outlined in the [Getting access tokens through the Developer Portal](/api-docs/static/oauth-ui-tokens.html) can also be used for Auth'n'Auth user tokens. The process for getting user tokens for both OAuth and Auth'n'Auth are basically the same, except that the **Auth'n'Auth** radio button will be selected in the User Tokens page.

![Banner image](/cms/img/authorization/gettingtokensgetting_tokens_for_applications_with_multiple_users5.jpg)
...
Once the processes described in  [Client/Desktop Applications](/api-docs/static/get_auth_n_auth_tokens.html#Option) or [Web/Server Applications](/api-docs/static/get_auth_n_auth_tokens.html#Option2) have been completed, from your desktop or web application use **FetchToken** to retrieve tokens for its users.
...
**FetchToken** is described in reference detail in [FetchToken](/Devzone/XML/docs/Reference/eBay/FetchToken.html).
...
[Authorizing API requests](#Authoriz)  
[Standardizing on OAuth](#Standard)  
[OAuth scopes and the traditional APIs](#OAuth)  
[Implementing OAuth instead of Auth'n'Auth](#Implemen)
...
Before the introduction of the eBay RESTful APIs, all requests to the traditional APIs require you to authorize your requests with a system that's known as Auth'n'Auth (which stands for _authentication and authorization_).

As you begin to use the new eBay APIs, you'll find the APIs require you to use OAuth _access tokens_ for authorization. This can be confusing because it appears that you need to use two separate authorization mechanisms if you want to use multiple eBay APIs.
...
All of the above APIs require user access tokens created with [The authorization code grant flow](/api-docs/static/oauth-authorization-code-grant.html). Later in this section, the methods for passing in OAuth tokens for each of these APIs are explained.

Like Auth'n'Auth, the OAuth authorization code grant flow involves delegation via a _user consent_. With this, your application must redirect the user to a **Grant Application Access Page** so they can consent to the eBay terms and conditions of use. And now that some traditional APIs support OAuth for authorization, you can get a user's consent just once, after which you can call any of the APIs that support OAuth for the eBay user.
...
**Note:** The Platform Notification calls **GetNotificationPreferences** and **SetNotificationPreferences** work with OAuth as outlined above, however your application needs to be white listed in order to receive platform notifications. Please contact [Developer Technical Support](/api-docs/static/gs_get-support.html) for details on getting your application white listed.

###### Using OAuth in Post Order API requests

The Post Order API uses the **Authorization** HTTP request header to pass the OAuth token with your REST request. See [Post-Order API – Making a Call](/devzone/post-order/concepts/MakingACall.html) for more information about using the Authorization header.
